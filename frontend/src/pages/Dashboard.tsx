...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl">
      {/* Welcome banner */}
      <div className="bg-gradient-to-r from-[var(--nora-pink)] to-[var(--nora-pink-dark)] rounded-2xl p-6 md:p-8 mb-6 md:mb-8 text-white">
        <h2 className="text-xl md:text-2xl font-light">Willkommen, {userName}</h2>
      ...
      {courses.length === 0 ? (
        <div className="bg-white rounded-2xl p-6 md:p-8 text-center text-gray-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {courses.map((course) => (
            <Link
              key={course.id}
              to={`/course/${course.id}`}
              className="bg-gradient-to-r from-[var(--nora-pink)] to-[var(--nora-pink-dark)] rounded-2xl p-5 md:p-8 flex items-center justify-between text-white hover:shadow-lg transition-shadow group gap-4"
            >
              <div className="min-w-0">
                <p className="text-xs md:text-sm opacity-80 uppercase tracking-wider">Kurs</p>
                <h4 className="text-lg md:text-xl font-semibold mt-1">{course.title}</h4>
                {course.description && (
