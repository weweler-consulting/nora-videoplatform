...
async def test_dismiss_deletes_lesson_and_video(client, session, monkeypatch):
    ...
    r = await client.post("/api/v1/live-calls/dismiss", data={"import_id": imp.id, "token": tok})
...
@pytest.mark.asyncio
async def test_dismiss_refuses_published(client, session, monkeypatch):
    course = Course(title="K", is_active=True); session.add(course); await session.commit()
    module = Module(course_id=course.id, title="M", sort_order=0); session.add(module); await session.commit()
    section = Section(module_id=module.id, title="S", sort_order=0); session.add(section); await session.commit()
    lesson = Lesson(section_id=section.id, title="L", type="video", video_url="https://iframe.mediadelivery.net/embed/1/v", is_published=True, sort_order=0)
    session.add(lesson); await session.commit()
    series = LiveCallSeries(course_id=course.id, recording_name_prefix="P"); session.add(series); await session.commit()
    imp = LiveCallImport(series_id=series.id, drive_file_id="f9", recording_name="P - 2026/10/02 19:14 WEST - Recording",
                         occurrence_at=datetime(2026, 10, 2, 19, 14), status="published", module_id=module.id, lesson_id=lesson.id)
    session.add(imp); await session.commit()
    lesson_id = lesson.id
    ...
    # GET-Bestätigung zeigt Hinweis statt Button
    tok = create_action_token(imp.id, "dismiss")
    g = await client.get(f"/api/v1/live-calls/dismiss?import_id={imp.id}&token={tok}")
    assert g.status_code == 200 and "nicht möglich" in g.text

    # POST verwirft NICHT — Lektion + Video bleiben
    r = await client.post("/api/v1/live-calls/dismiss", data={"import_id": imp.id, "token": tok})
    assert r.status_code == 200
    session.expunge_all()
    assert (await session.execute(_select(Lesson).where(Lesson.id == lesson_id))).scalar_one_or_none() is not None
    assert deleted == []
