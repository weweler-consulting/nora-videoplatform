...
async def test_fills_dated_placeholder(session):
    ...
    section_id, module_id, created = await resolve_target_section(session, course.id, datetime(2026, 10, 2, 19, 14))
    assert module_id == placeholder.id and created is False  # Datum traf den Platzhalter
...
async def test_creates_new_module_when_no_placeholder(session):
    ...
    section_id, module_id, created = await resolve_target_section(session, course.id, datetime(2026, 10, 9, 19, 14))
...
async def test_placeholder_with_video_lesson_is_not_reused(session):
    ...
    _section_id, module_id, created = await resolve_target_section(session, course.id, datetime(2026, 10, 2, 19, 14))
    assert module_id != mod.id and created is True  # belegter Platzhalter → neues Modul


@pytest.mark.asyncio
async def test_same_daymonth_different_year_not_matched(session):
    course = Course(title="K", is_active=True); session.add(course); await session.commit()
    wrong_year = Module(course_id=course.id, title="Live Call 02.10.2025", sort_order=0)
    session.add(wrong_year); await session.commit()

    _section_id, module_id, created = await resolve_target_section(session, course.id, datetime(2026, 10, 2, 19, 14))
    assert module_id != wrong_year.id and created is True  # 2025 darf NICHT für 2026 matchen
